...
-- $Id: tab_stat.sql 10 2008-11-11 10:25:06Z oracle $
--
-- Display lot of detail information for one table
--
-- Parameter 1: Owner
-- Parameter 2: Table-Name
--
-- Metalink Note:31412.1 Subject: 	SCRIPT - Select to show Optimizer Statistics for CBO
-- Modified for default parameter
-- Column DENSITY = density*1000
...
set termout on
column TABLE_NAME heading "Tables owned by &1" format a30
--select table_name from dba_tables where owner=upper('&1') order by 1
--/
undefine table_name
undefine owner
prompt
--accept owner prompt 'Please enter Name of Table Owner (Null = &1): '
...
column BLOCKS heading "Blocks" format 99,999,990
...
column DENSITYK heading "Density" format 999.990
...
where 
    table_name = upper('&2')
and table_owner = upper(nvl('&1',user))
/
break on index_name
select
    i.INDEX_NAME,
...
where 
    i.table_name = upper('&2')
and i.table_owner = upper(nvl('&1',user))
